 ...
 */
  ...
  *
  * This is a PGR semestral work project
  *
  */
...
/// <summary>
/// \brief function to move the camera
/// </summary>
/// <returns></returns>
...
/// <summary>
/// \brief check if position is in bounds of the scene and not interlaping with other objects
...
bool check_bounds(glm::vec3 position) {
	...
	if (pow(pow(position.x - fire_position.x, 2)+ pow(position.z - fire_position.z, 2), 0.5f) < FIRE_SIZE) {
		return false;
	}
	return true;
}
...
void drawScene(void)
{
	...
	for (ObjectInstance* object : objects) {   // for (auto object : objects) {
		if (object != nullptr && object->isPresent) {
			if (object->isPresent) {
...
void timerCb(int)
{
	...
	float deltaTime = (time - gameState.last_update)/1000; //seconds from miliseconds
...
void initApplication() {
	...
	loadShaderPrograms();
	CHECK_GL_ERROR();
	//objects.push_back(new Smoke(&commonShaderProgram));
	gameState.fire = new Fire(&commonShaderProgram, &fireShaderProgram);
	gameState.firewood = new Firewood(WOOD_MODEL, WOOD_SPACING, WOOD_ROTATION_VEC, WOOD_ROTATION_size, WOOD_TRASLATION, WOOD_SCALE, &commonShaderProgram);
	...
	objects.push_back(new Vegetation(GRASS_MODEL, GRASS_SPACING, GRASS_ROTATION_VEC, GRASS_ROTATION_size, GRASS_TRASLATION, GRASS_SCALE, &commonShaderProgram));
	objects.push_back(new Vegetation(BUSH_MODEL, BUSH_SPACING, BUSH_ROTATION_VEC, BUSH_ROTATION_size, BUSH_TRASLATION, BUSH_SCALE, &commonShaderProgram));
	objects.push_back(new Vegetation(FERN_MODEL, FERN_SPACING, FERN_ROTATION_VEC, FERN_ROTATION_size, FERN_TRASLATION, FERN_SCALE, &commonShaderProgram));
	objects.push_back(new Vegetation(PLANT_MODEL, PLANT_SPACING, PLANT_ROTATION_VEC, PLANT_ROTATION_size, PLANT_TRASLATION, PLANT_SCALE, &commonShaderProgram));
	objects.push_back(gameState.firewood);
