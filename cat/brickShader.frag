...
void setupLights() {
  ...
  sun.position = (Vmatrix * vec4(cos(time * sunSpeed), 1.0, sin(time * sunSpeed), 0.0)).xyz;
...
void main() {
 ...
 if(reflectorOn==0){
  ...
  if(material.useTexture){
	...
	//color_f =  mix(fogColor_f, color_f *texColor,fog);
	color_f =  mix(fogColor_f, texColor,fog);
  ...
  //color_f = vec4(sun.position,1.0f);
  //color_f = directionalLight(sun, material, vertexPosition, normal);
}
